...
def askingQarnot(task_name, bucket, nb_instances, cmd):
	...
	try:
		...
		task.constants['DOCKER_TAG'] = "dock_multi_original"
...
args_dock=args_setup+"|"+args_simul
...
def creation_bucket():
	output_bucket = conn.create_bucket("dock_multi")
	for mol in liste:
...
askingQarnot("dock_multi", creation_bucket(), len(liste), "python"+" ../prog/dock.py " +" "+ str(swarm)+ " "+str(glow)+" "+str(steps)+" "+args_dock)
