...
def askingQarnot(task_name, bucket, nb_instances, cmd):
	...
	try:
		...
		task.constants['DOCKER_TAG'] = "dock_original"
...
def creation_bucket(mol):
	output_bucket = conn.create_bucket("dock_original")
...
for pdb in liste:
	...
	askingQarnot("setup_original_"+pdb, creation_bucket(pdb), 1, "python"+" ../prog/setup_dock.py " +" "+ str(swarm)+ " "+str(glow)+" "+args_setup)
	askingQarnot("dock_original_"+pdb, conn.create_bucket("dock_original"), 1, "python"+" ../prog/simul_dock.py " +" "+ str(swarm)+ " "+str(glow)+" "+str(steps)+" "+args_simul)
