...
def askingQarnot(task_name, bucket, nb_instances, cmd):
	...
	try:
		...
		task.constants['DOCKER_TAG'] = "dock_single"
...
def creation_bucket(mol):
	output_bucket = conn.create_bucket("dock_single")
...
for pdb in liste:
	...
	askingQarnot("setup_single_"+pdb, creation_bucket(pdb), 1, "python"+" ../prog/setup_dock.py " +" "+ str(swarm)+ " "+str(glow)+" "+args_setup)
	askingQarnot("dock_single_"+pdb, conn.create_bucket("dock_single"), swarm, "python"+" ../prog/simul_dock.py " +" "+ str(swarm)+ " "+str(glow)+" "+str(steps)+" "+args_simul)
	total_time=time.time()-start_time
	exe_time.append(total_time)

with open("temps_exe_1s_1p.result", "w+")as file:
