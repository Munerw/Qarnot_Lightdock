...
INSTANCE_ID=int(os.environ.get('INSTANCE_ID'))
INSTANCE_COUNT=os.environ.get('INSTANCE_COUNT')

args=[]

with open("lig.index") as input_file:
	for row in input_file:
		args.append(row.split("\n")[0])
...
DEFAULT_OUTPATH=args[FILE_ID]+"/"
if not os.path.isdir(DEFAULT_OUTPATH):
	os.mkdir(DEFAULT_OUTPATH)

swarm_id=[]
arg_setup={}

if(os.path.exists(DEFAULT_OUTPATH+"setup.json")):
	with open(DEFAULT_OUTPATH+"setup.json") as input_file:
		setup=json.load(input_file)
		for i in range(16):
			if(setup["swarms"]>16*INSTANCE_ID+i):
				swarm_id.append(16*INSTANCE_ID+i)

SWARM_ID = swarm_id

# Default file names and folders
DEFAULT_POSITIONS_FOLDER = DEFAULT_OUTPATH+"init"       # Folder which contains the initial_positions files for each cluster
GSO_OUTPUT_FILE = "gso_%s.out"
DEFAULT_SWARM_FOLDER = DEFAULT_OUTPATH+"swarm_"     # Folder where GSO execution for a given cluster will be stored
DEFAULT_SETUP_FILE = DEFAULT_OUTPATH+"setup.json"
...
CLUSTER_DEFAULT_NAME = "cluster_swarm"
CLUSTER_ANALYSIS_FILE = CLUSTER_DEFAULT_NAME + ".repr"
CLUSTER_REPRESENTATIVES_FILE = DEFAULT_OUTPATH + CLUSTER_DEFAULT_NAME + DEFAULT_REPRESENTATIVES_EXTENSION
